...
router.post('/api/accommodations', async (req, res, next) => {
	try {
		...
		const createdAccommodation = await Accommodation.create({ name, description, img, category, externalUrl, affiliateUrl, city });
...
router.put('/api/accommodations/:accommodationId', async (req, res, next) => {
	try {
		...
		const updatedAccommodation = await Accommodation.findByIdAndUpdate(
			accommodationId,
			{ name, description, img, category, externalUrl, affiliateUrl },
			{ new: true }
		);
