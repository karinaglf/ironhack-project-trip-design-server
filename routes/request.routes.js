...
router.post('/api/requests', async (req, res, next) => {
  try {
    ...
    const foundedUser = await User.findById(requestedBy).populate(
      'requestedTrips'
    );
    console.log(`foundedUser in requests Routes`, foundedUser);
  } catch (error) {
...
router.delete('/api/requests/:requestId', async (req, res, next) => {
  try {
    ...
    const foundedRequest = await Requests.findById(requestId).populate(
      'requestedBy'
    );
    ...
    const updatedCreatorUser = await User.findByIdAndUpdate(
      foundedRequest.requestedBy,
      { $pull: { requestedTrips: foundedRequest._id } }
    );
